...
// busca mock
$idx = null; $piloto = null;
foreach ($_SESSION['mock_pilotos'] as $k => $p) {
  if ($p['id'] === $id) { $idx = $k; $piloto = $p; break; }
}

/* ===== FUTURO (PDO)
require_once dirname(__DIR__, 2) . '/includes/db.php';
$st = $pdo->prepare("SELECT * FROM pilotos WHERE id = ?");
$st->execute([$id]);
$piloto = $st->fetch(PDO::FETCH_ASSOC);
*/

if (!$piloto) { header('Location: /admin/pilotos/listar.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($nome === '' || $numero <= 0) {
    ...
    // MOCK update
    $_SESSION['mock_pilotos'][$idx] = array_merge($piloto, [
      'nome'       => $nome,
      'numero'     => $numero,
      'pais'       => $pais,
      'equipe'     => $equipe,
    ...
    $piloto = $_SESSION['mock_pilotos'][$idx];
    $ok = 'Piloto atualizado (mock).';

    /* ===== FUTURO (PDO)
    require_once dirname(__DIR__, 2) . '/includes/db.php';
    $sql = "UPDATE pilotos SET nome=:nome, numero=:numero, pais=:pais, equipe=:equipe, foto_url=:foto, nascimento=:nasc
            WHERE id=:id";
    $st = $pdo->prepare($sql);
    $st->execute([
      ':nome'=>$nome, ':numero'=>$numero, ':pais'=>$pais, ':equipe'=>$equipe, ':foto'=>$foto, ':nasc'=>$nasc, ':id'=>$id
    ]);
    $ok = 'Piloto atualizado no banco.';
    */
  }
}
?>
...
<main class="mx-auto max-w-3xl px-4 py-8">
  ...
  <form method="post" class="grid md:grid-cols-2 gap-4 bg-white border rounded-2xl p-6 shadow">
    ...
    <div>
      ...
      <input name="equipe" class="mt-1 w-full rounded-lg border px-3 py-2" value="<?= htmlspecialchars($piloto['equipe'] ?? '') ?>">
    </div>
    <div>
      <label class="block text-sm font-medium">Data de nascimento</label>
      <input type="date" name="nascimento" class="mt-1 w-full rounded-lg border px-3 py-2" value="<?= htmlspecialchars($piloto['nascimento'] ?? '') ?>">
    </div>
    <div class="md:col-span-2">
